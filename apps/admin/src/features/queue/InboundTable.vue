<script setup lang="ts">
import { CornerDownLeft } from 'lucide-vue-next'

import AppButton from '@/components/AppButton.vue'
import AppPagination from '@/components/AppPagination.vue'
import AppTable from '@/components/AppTable.vue'
import EmptyState from '@/components/EmptyState.vue'
import StatusChip from '@/components/StatusChip.vue'
import TimeAgo from '@/components/TimeAgo.vue'
import type { InboundCommandListItem } from '@/contracts/queue'
import { inboundCommandVisual, inboundStateVisual } from './queue-status'

defineProps<{
  rows: InboundCommandListItem[]
  loaded: boolean
  refreshing: boolean
  page: number
  total: number | null
  hasNext: boolean
  hasPrevious: boolean
  hasFilters: boolean
}>()

const emit = defineEmits<{
  open: [row: InboundCommandListItem]
  next: []
  previous: []
  clearFilters: []
}>()

const columns = [
  { key: 'command', label: '命令' },
  { key: 'state', label: '状态', width: '120px' },
  { key: 'senderHint', label: '发送者' },
  { key: 'attemptCount', label: '尝试', align: 'right' as const, width: '70px' },
  { key: 'createdAt', label: '创建', width: '110px' },
  { key: 'expiresAt', label: '过期', width: '110px' },
  { key: 'errorCode', label: '最后错误', width: '160px' },
]
</script>

<template>
  <EmptyState
    v-if="loaded && rows.length === 0"
    :icon="CornerDownLeft"
    title="没有匹配的入站命令"
    :description="hasFilters ? '当前筛选条件下没有入站命令。' : '当前还没有任何入站命令。'"
  >
    <template v-if="hasFilters" #action>
      <AppButton variant="secondary" size="sm" @click="emit('clearFilters')">清空筛选</AppButton>
    </template>
  </EmptyState>
  <div v-else class="queue-table-shell">
    <AppTable
      :columns="columns"
      :rows="rows"
      :row-key="(row: InboundCommandListItem) => row.id"
      :loading="refreshing"
      :interactive-rows="false"
      :aria-label="`入站命令列表（${rows.length} 条）`"
      data-testid="inbound-table"
    >
      <template #cell-command="{ row }">
        <button
          type="button"
          class="queue__detail-link queue-table__detail-link"
          :aria-label="`查看入站命令 ${row.id} 详情`"
          @click="emit('open', row)"
        >
          {{ inboundCommandVisual(row.command) }}
        </button>
      </template>
      <template #cell-senderHint="{ row }">
        <span class="queue-table__mono">{{ row.senderHint }}</span>
      </template>
      <template #cell-state="{ row }">
        <StatusChip
          :tone="inboundStateVisual(row.state).tone"
          :label="inboundStateVisual(row.state).label"
        />
      </template>
      <template #cell-attemptCount="{ row }">
        <span class="tabular">{{ row.attemptCount }}</span>
      </template>
      <template #cell-createdAt="{ row }"><TimeAgo :timestamp="row.createdAt" /></template>
      <template #cell-expiresAt="{ row }"><TimeAgo :timestamp="row.expiresAt" /></template>
      <template #cell-errorCode="{ row }">
        <span class="queue-table__error mono break-anywhere">{{ row.errorCode ?? '—' }}</span>
      </template>
    </AppTable>
    <AppPagination
      :page="page"
      :page-size="50"
      :total="total"
      :has-next="hasNext"
      :has-prev="hasPrevious"
      @next="emit('next')"
      @prev="emit('previous')"
    />
  </div>
</template>

<style scoped>
.queue-table__detail-link {
  padding: 0;
  color: var(--pd-text-link);
  background: transparent;
  border: 0;
  font: inherit;
  text-align: start;
  cursor: pointer;
  text-decoration: underline;
  text-underline-offset: 2px;
}

.queue-table__detail-link:focus-visible {
  outline: 2px solid var(--pd-border-focus);
  outline-offset: 2px;
}

.queue-table__mono {
  color: var(--pd-text-muted);
  font-family: var(--pd-font-code);
  font-size: var(--pd-font-size-11);
}

.queue-table__error {
  color: var(--pd-feedback-danger);
  font-size: var(--pd-font-size-11);
}

.queue-table-shell :deep(.app-table) {
  border-end-start-radius: 0;
  border-end-end-radius: 0;
}

.queue-table-shell :deep(.app-pagination) {
  padding: var(--pd-space-8) var(--pd-space-12);
  background: var(--pd-container-header-bg);
  border: 1px solid var(--pd-border-separator);
  border-top: 0;
  border-end-start-radius: var(--pd-radius-md);
  border-end-end-radius: var(--pd-radius-md);
}
</style>
